...
test.describe("Pet endpoint : GET Method", () => {
  ...
  test("should retrieve pet information by ID for existing pet", async ({
    request,
  }) => {
  ...
  test.describe("Get Pet Information by Status", () => {
    test("should retrieve pet information by status", async ({ request }) => {
      const response = await request.get(
        `pet/findByStatus?status=${PetStatus.available}`
      );
      expect(response.status()).toBe(200);
